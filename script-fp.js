...
/* ══════════════════════════════════════════════════════
   Gallery — every AI-generated image AND video (AI Studio tab + shop
   "place in scene") lands here so it can be reviewed or downloaded later
   without regenerating it. Session-only, in-memory — resets on page
   reload, same lifetime as _aiImages/shopState.
   ══════════════════════════════════════════════════════ */
let galleryImages = [];
...
function galleryAdd(b64, mime, label) {
  const type = (mime || "").startsWith("video/") ? "video" : "image";
  galleryImages.unshift({ b64, mime: mime || "image/png", label: label || "Generato", ts: Date.now(), type });
  const badge = document.getElementById("gallery-btn-badge");
  if (badge) {
    badge.textContent = galleryImages.length;
    badge.classList.add("show");
  }
  if (document.getElementById("gallery-modal")?.classList.contains("open")) galleryRender();
}

/* Same as galleryAdd, but takes a video Blob directly (Veo results come
   back as Blobs, not base64) and awaits the base64 conversion. */
async function galleryAddVideoBlob(blob, label) {
  try {
    const b64 = await blobToBase64(blob);
    galleryAdd(b64, blob.type || "video/mp4", label);
  } catch (e) {
    console.error("galleryAddVideoBlob failed", e);
  }
}
...
function galleryRender() {
  ...
  if (!galleryImages.length) {
    grid.innerHTML = '<div id="gallery-empty">Ancora nessuna immagine o video — generane uno dal pannello IA o dal negozio.</div>';
    return;
  }
  grid.innerHTML = galleryImages.map((img, i) => `
    <div class="gallery-item">
      ${img.type === "video"
        ? `<video src="data:${img.mime};base64,${img.b64}" muted loop playsinline preload="metadata" onclick="galleryPreview(${i})"></video>
           <div class="gallery-item-play">▶</div>`
        : `<img src="data:${img.mime};base64,${img.b64}" alt="${img.label}" onclick="galleryPreview(${i})" />`}
      <div class="gallery-item-label">${img.label}</div>
      <button class="gallery-dl-btn" title="Scarica" onclick="event.stopPropagation();galleryDownload(${i})">
...
function galleryPreview(i) {
  ...
  const lightboxVideo = document.getElementById("ai-lightbox-video");
  if (img.type === "video") {
    lightboxVideo.src = `data:${img.mime};base64,${img.b64}`;
    lightboxVideo.classList.add("open");
    lightboxImg.classList.remove("open");
    lightboxVideo.play().catch(() => {});
  } else {
    lightboxImg.src = `data:${img.mime};base64,${img.b64}`;
...
function galleryDownload(i) {
  ...
  if (!img) return;
  let ext = "jpg";
  if (img.mime.includes("png")) ext = "png";
  else if (img.mime.includes("webm")) ext = "webm";
  else if (img.type === "video") ext = "mp4";
  const a = document.createElement("a");
  a.href = `data:${img.mime};base64,${img.b64}`;
  a.download = (img.label || (img.type === "video" ? "video" : "image")).toLowerCase().replace(/[^a-z0-9]+/g, "-") + "-" + img.ts + "." + ext;
  a.click();
}
