...
/* ══════════════════════════════════════════════════════
   Gallery — every AI-generated image (AI Studio tab + shop "place in
   scene") lands here so it can be reviewed or downloaded later without
   regenerating it. Session-only, in-memory — resets on page reload,
   same lifetime as _aiImages/shopState.
   ══════════════════════════════════════════════════════ */
let galleryImages = [];

function galleryAdd(b64, mime, label) {
  galleryImages.unshift({ b64, mime: mime || "image/png", label: label || "Generated", ts: Date.now() });
...
function galleryRender() {
  ...
  if (!galleryImages.length) {
    grid.innerHTML = '<div id="gallery-empty">No images yet — generate one from the AI panel or the shop.</div>';
    return;
  }
...
function galleryPreview(i) {
  ...
  if (!img) return;
  document.getElementById("ai-lightbox-img").src = `data:${img.mime};base64,${img.b64}`;
...
function galleryDownload(i) {
  ...
  a.download = (img.label || "image").toLowerCase().replace(/[^a-z0-9]+/g, "-") + "-" + img.ts + "." + ext;
  a.click();
}
